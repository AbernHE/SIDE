...
class BIN_Interaction_Flat(nn.Sequential):
    ...
    def __init__(self, **config):
        ...
        self.max_d = config['max_RNA1_seq']
        self.max_p = config['max_RNA2_seq']
        ...
        self.input_dim_RNA1 = config['input_dim_RNA1']
        self.input_dim_RNA2 = config['input_dim_RNA2']
        self.gpus = 1
        self.n_layer = 2
        self.temperature = 2
        #encoder
        ...
        self.demb = Embeddings(self.input_dim_RNA1, self.emb_size, self.max_d, self.dropout_rate)
        self.pemb = Embeddings(self.input_dim_RNA2, self.emb_size, self.max_p, self.dropout_rate)
        ...
        self.decoder_seq = nn.Sequential(
            nn.Linear(self.max_d*self.emb_size*2, 512),
            nn.ReLU(True),
        ...
        self.decoder_contrast_inter = nn.Sequential(
            nn.Linear(self.flatten_dim, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, 128),
        ...
        self.decoder_contrast_seq = nn.Sequential(
            nn.Linear(self.max_d*self.emb_size*2, 512),
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Linear(512, 128),
    ...
    def forward(self, d, p, d_mask, p_mask):
        ...
        p_aug = torch.unsqueeze(p_encoded_layers, 1).repeat(1, self.max_d, 1, 1) # repeat along RNA1 size
        
        # seq_emb
        d_seqe = d_encoded_layers.view(self.batch_size,-1)
        p_seqe = p_encoded_layers.view(self.batch_size,-1)
        comb_seqe = torch.cat((d_seqe, p_seqe), dim = 1)


        i = d_aug  * p_aug # interaction
        # print(i)
        i_v = i.view(int(self.batch_size/self.gpus), -1, self.max_d, self.max_p) 
        # batch_size x embed size x max_RNA1_seq_len x max_protein_seq_len
        ...
        #contrast feature of the both
        contrast_inter_emb = self.decoder_contrast_inter(f)
        contrast_seq_emb = self.decoder_contrast_seq(comb_seqe)
        z_i = F.normalize(contrast_inter_emb, dim=1)
        z_j = F.normalize(contrast_seq_emb, dim=1)
        ...
        def l_ij(i, j):
            ...
            one_for_not_i = torch.ones((2 * self.batch_size,)).scatter_(0, torch.tensor([i]), 0.0)
            # if self.verbose:
            #     print(f"1{{k!={i}}}", one_for_not_i)
            one_for_not_i = one_for_not_i.to('cuda')
        ...
        for k in range(0, self.batch_size):
            loss += l_ij(k, k + self.batch_size) + l_ij(k + self.batch_size, k)

        loss = 1.0 / (2 * self.batch_size) * loss

        feature = torch.cat((contrast_inter_emb, contrast_seq_emb), 1)
        ...
        score_seq = self.decoder_seq(comb_seqe)
        score = self.decoder(f)
        fin_score = pred + score_seq + score
        return fin_score, loss    
...
class Embeddings(nn.Module):
    """Construct the embeddings from protein/RNA2, position embeddings.
    """
    ...
    def forward(self, input_ids):
        ...
        words_embeddings = self.word_embeddings(input_ids)   #[16,50,384]
        position_embeddings = self.position_embeddings(position_ids)   #[16,50,384]
...
class ProtectPredictor(nn.Module):
    def __init__(self):
        super(ProtectPredictor, self).__init__()
